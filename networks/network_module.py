...
class DWConv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', groups = 1, sn = False):
        ...
        self.conv = nn.Sequential(
            ...
            # pw
            Conv2dLayer(in_channels, out_channels, kernel_size = 1, stride = 1, padding = 0, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, groups = 1, sn = sn)
        )
...
class DWTransposeConv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', groups = 1, sn = False, scale_factor = 2):
        ...
        self.conv = nn.Sequential(
            ...
            Conv2dLayer(in_channels, out_channels, kernel_size = 1, stride = 1, padding = 0, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, groups = 1, sn = sn),
            # dw
            TransposeConv2dLayer(out_channels, out_channels, kernel_size, stride, padding, dilation, pad_type, activation, norm, groups = out_channels, sn = sn, scale_factor = scale_factor),
            # pw-linear
            Conv2dLayer(out_channels, out_channels, kernel_size = 1, stride = 1, padding = 0, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, groups = 1, sn = sn)
        )
